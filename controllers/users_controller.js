...
exports.find = async (req, res) => {
  try {
    ...
    let query = `SELECT * FROM users WHERE id IN (${params.fill("?")})`;
...
exports.findAll = async (req, res) => {
  try {
    await db.all(`SELECT * FROM users`, (error, rows) => {
      if (error) return res.status(400).json(error);
      return res.status(200).json(rows);
    });
  } catch (error) {
    ...
    await db.all(
      ...
      (error, rows) => {
        ...
        for (let key in req.body) {
          let stm = db.prepare(
            `Update users SET ${key} = ${req.body[key]} WHERE id = (?)`,
            req.params.id
          );
          stm.run();
          stm.finalize();
  ...
  try {
    await db.all(
      ...
      (error, rows) => {
        ...
        let stm = db.prepare(`DELETE FROM users WHERE id = (?)`, req.params.id);
        stm.run();
        stm.finalize();
