...
exports.find = async (req, res) => {
  try {
    ...
    let query = `SELECT * FROM settings WHERE server_id IN (${params.fill(
      "?"
    )})`;
...
exports.findAll = async (req, res) => {
  try {
    await db.all(`SELECT * FROM settings`, (error, rows) => {
      if (error) return res.status(400).json(error);
      return res.status(200).json(rows);
    });
  } catch (error) {
...
exports.insert = async (req, res) => {
  try {
    if (
      !Object.keys(req.body).includes("server_id") ||
      Object.keys(req.body).length != 1
    )
      return res.status(403).json(`Invalid body.`);

    await db.all(
      `SELECT server_id FROM settings WHERE server_id = ?`,
      req.body.id,
      (error, rows) => {
        if (error) return res.status(400).json(error);
    ...
    await db.all(
      ...
      (error, rows) => {
        ...
        for (let key in req.body) {
          let stm = db.prepare(
            `Update settings SET ${key} = ${req.body[key]} WHERE server_id = (?)`,
            req.params.id
          );
          stm.run();
          stm.finalize();
  ...
  try {
    await db.all(
      ...
      (error, rows) => {
        ...
        let stm = db.prepare(
          `DELETE FROM settings WHERE server_id = (?)`,
          req.params.id
        );
        stm.run();
        stm.finalize();
