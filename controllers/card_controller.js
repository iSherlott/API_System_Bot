...
exports.find = async (req, res) => {
  try {
    ...
    let query = `SELECT * FROM cards WHERE card_id IN (${params.fill("?")})`;
...
exports.findAll = async (req, res) => {
  try {
    await db.all(`SELECT * FROM cards`, (error, rows) => {
      if (error) return res.status(400).json(error);
      return res.status(200).json(rows);
    });
  } catch (error) {
...
exports.insert = async (req, res) => {
  try {
    if (
      !Object.keys(req.body).includes("card_id") ||
    ...
    await db.all(
      ...
      (error, rows) => {
        ...
        let stm = db.prepare(
          `INSERT INTO users ("card_id") VALUES (?);`,
          req.body.id
        );
        stm.run();
        stm.finalize();
    ...
    await db.all(
      ...
      (error, rows) => {
        ...
        for (let key in req.body) {
          let stm = db.prepare(
            `Update cards SET ${key} = ${req.body[key]} WHERE card_id = (?)`,
            req.params.id
          );
          stm.run();
          stm.finalize();
